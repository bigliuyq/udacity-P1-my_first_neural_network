...
class NeuralNetwork(object):
    def __init__(self, input_nodes, hidden_nodes, output_nodes, learning_rate):
        ...
        # Initialize weights  ##这个对权重的初始化很有用，也很有必要，不能讲权重全初始化为0
    ...
    def train(self, features, targets):
        ...
        '''
        n_records = features.shape[0]#每一行代表一个样本，每一列代表一个特征
        delta_weights_i_h = np.zeros(self.weights_input_to_hidden.shape)
        delta_weights_h_o = np.zeros(self.weights_hidden_to_output.shape)
        for X, y in zip(features, targets):#每次取一个样本
            ...
            final_outputs = final_inputs # signals from final output layer #输出层的激活函数y=x
            ...
            output_error_term = error* 1.0#输出层激活函数y=x，导数为1
            # TODO: Calculate the hidden layer's contribution to the error     #注意这一项的矩阵维度
            hidden_error = np.dot(output_error_term,self.weights_hidden_to_output.T)#隐含层的误差为输出层误差项与它们之间的权重乘积的和
            ...
            delta_weights_h_o += output_error_term*hidden_outputs[:,None]#注意维度的问题，将输入项转换为列向量
...
# #### 请将你的答案填写在下方
# 
# 先说一下结果：调试了很多组超参数，总的看下来，训练误差0.07左右，验证误差0.17左右。
# 对于测试集的预测效果如图所示，前半段的预测效果还不错，可后半段的预测效果不佳。
# 可见泛化能力不够好，应该加正则化策略，提高泛化能力。另外，只用了一层隐含层，还可增加隐含层，提高学习能力。
# 
# 另外，在编写代码的过程中，对反向传播计算梯度的过程，理解的不够清晰，对维度搞不太清楚，课程中讲的也不太清楚，
# 希望能有一份详细的推导教程，包含维度细节，而不仅仅只有公式。
# 
# 
